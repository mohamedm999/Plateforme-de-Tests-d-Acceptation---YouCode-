...
{{--
    <div class="p-4 mb-6 bg-yellow-100 border border-yellow-300 rounded shadow">
        <h3 class="font-bold">Debug Info:</h3>
        <p>Authenticated: {{ auth()->check() ? 'Yes' : 'No' }}</p>
        @if(auth()->check())
            <p>User ID: {{ auth()->id() }}</p>
            <p>User Email: {{ auth()->user()->email }}</p>
            <p>User has roles relationship: {{ method_exists(auth()->user(), 'roles') ? 'Yes' : 'No' }}</p>
            @php
                try {
                    if (auth()->user()->roles) {
                        echo "<p>Roles: " . auth()->user()->roles->pluck('name')->implode(', ') . "</p>";
                    } else {
                        echo "<p>No roles found or relationship not loaded</p>";
                    }
                } catch(\Exception $e) {
                    ...
                    @if (auth()->user()->hasAnyRole(['candidat', 'candidate']))
                        <div class="pl-4 border-l-4 border-blue-500">
                            ...
                            <div class="grid grid-cols-1 gap-4 mt-6 md:grid-cols-2">
                                ...
                                <a href="{{ route('candidate.quizzes.index') }}"
