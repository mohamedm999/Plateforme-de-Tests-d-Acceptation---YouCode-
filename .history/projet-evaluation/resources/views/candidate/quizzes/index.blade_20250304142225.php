            ...
            <div class="overflow-hidden bg-white shadow-xl rounded-xl dark:bg-gray-800">
                <div class="p-8">
                    ...
                    <div class="grid gap-6 md:grid-cols-2">
                                        ...
                                        <div class="mt-auto">
                                            @if (in_array($quiz->id, $attemptedQuizzes))
                                                <a href="{{ route('quizzes.results', \App\Models\QuizAttempt::where('user_id', Auth::id())->where('quiz_id', $quiz->id)->whereNotNull('completed_at')->first()->id) }}"
                                                    class="block w-full px-4 py-3 text-sm font-medium text-center text-blue-700 transition-colors border border-blue-600 rounded-md hover:bg-blue-50 dark:text-blue-400 dark:border-blue-400 dark:hover:bg-blue-900/30">
                                                    View Results
                                                </a>
                                            @else
                                                <a href="{{ route('quizzes.show', $quiz->id) }}" 
                                                   class="block w-full px-4 py-3 text-sm font-medium text-center text-white transition-colors bg-blue-600 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:bg-blue-700 dark:hover:bg-blue-600">
                                                    Start Quiz
                                                </a>
                                            @endif
                                        </div>
                        ...
                        @empty
                            <div class="col-span-2 p-8 text-center rounded-lg bg-gray-50 dark:bg-gray-700">
                                <svg class="w-16 h-16 mx-auto text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"></path>
                                </svg>
                                <p class="mt-4 text-lg font-medium text-gray-700 dark:text-gray-300">No quizzes are currently available</p>
                                <p class="mt-2 text-gray-500 dark:text-gray-400">Check back later for new challenges to test your knowledge</p>
                            </div>
                        @endforelse
                    </div>
                </div>
