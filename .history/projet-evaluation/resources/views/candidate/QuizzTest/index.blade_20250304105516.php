                                ...
                                <div class="flex justify-between">
                                    ...
                                    <div>
                                        @if (in_array($quiz->id, $attemptedQuizzes))
                                            ...
                                            <a href="{{ route('candidate.quizzes.results', QuizAttempt::where('user_id', Auth::id())->where('quiz_id', $quiz->id)->whereNotNull('completed_at')->first()->id) }}"
                                                class="block px-4 py-2 mt-2 text-sm text-center text-white bg-blue-600 rounded-md hover:bg-blue-700">
                                                View Results
                                            </a>
                                        @else
                                            <a href="{{ route('candidate.quizzes.show', $quiz->id) }}"
                                                class="block px-4 py-2 text-sm text-center text-white bg-blue-600 rounded-md hover:bg-blue-700">
                                                Start Quiz
                                            </a>
                                        @endif
                                    </div>
