...
<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 flex items-center">
                        ...
                        <div>
                            ...
                            <div class="text-2xl font-semibold">{{ $documents->where('status', 'pending')->count() }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 flex items-center">
                        ...
                        <div>
                            ...
                            <div class="text-2xl font-semibold">{{ $documents->where('status', 'validated')->count() }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 flex items-center">
                        ...
                        <div>
                            <div class="text-sm font-medium text-gray-500">Rejetés</div>
                            <div class="text-2xl font-semibold">{{ $documents->where('status', 'rejected')->count() }}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Liste des documents</h3>
                    ...
                    @if ($documents->isEmpty())
                        <div class="text-center py-8">
                            ...
                            <div class="mt-6">
                                <a href="{{ route('candidate.documents.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                    Ajouter un document
                                </a>
                            </div>
                        </div>
                                        ...
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <div class="flex space-x-3">
                                                    <!-- View button -->
                                                    <button onclick="viewDocument('{{ $document->id }}')" class="text-indigo-600 hover:text-indigo-900" title="Voir">
                                                    ...
                                                    @if($document->status == 'pending' || $document->status == 'rejected')
                                                        <form action="{{ route('candidate.documents.destroy', $document->id) }}" method="POST" class="inline" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce document ?')">
                                                            @csrf
                                                            @method('DELETE')
                                                    ...
                                                    @if($document->status == 'rejected' && $document->admin_comment)
                                                        <button onclick="showRejectionReason('{{ addslashes($document->admin_comment) }}')" class="text-gray-500 hover:text-gray-700" title="Voir la raison du rejet">
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="p-4 bg-gray-50 rounded-md">
                            <div class="flex items-center mb-2">
                                ...
                                <h4 class="font-medium">Pièce d'identité</h4>
                            </div>
                            <p class="text-sm text-gray-600">Carte nationale d'identité, passeport ou permis de conduire valide.</p>
                        </div>
                        ...
                        <div class="p-4 bg-gray-50 rounded-md">
                            <div class="flex items-center mb-2">
                                ...
                                <h4 class="font-medium">Diplôme/Certificat</h4>
                            </div>
                            <p class="text-sm text-gray-600">Votre dernier diplôme ou certificat de formation pertinent pour la candidature.</p>
                        </div>
                        ...
                        <div class="p-4 bg-gray-50 rounded-md">
                            <div class="flex items-center mb-2">
                                ...
                                <h4 class="font-medium">Autres documents</h4>
                            </div>
                            <p class="text-sm text-gray-600">Tout autre document pertinent pour soutenir votre candidature (lettre de recommandation, etc).</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div id="document-modal" class="fixed inset-0 overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true" style="z-index: 50;">
                            ...
                            <div class="mt-4">
                                <div id="document-preview-container" class="h-96 flex items-center justify-center bg-gray-100 rounded-md">
                                    ...
                                    <div id="document-preview-pdf" class="text-center hidden">
                                        ...
                                        <a id="pdf-download-link" href="#" class="mt-4 inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" target="_blank">
                                            Télécharger le PDF
                                        </a>
                                    </div>
    ...
    <div id="rejection-modal" class="fixed inset-0 overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true" style="z-index: 50;">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true"></div>

            <span class="hidden sm:inline-block sm:align<!-- filepath: /c:/github/Plateforme-de-Tests-d-Acceptation---YouCode-/projet-evaluation/resources/views/candidate/Documents/index.blade.php -->
<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
...
<!-- Complete the rejection reason modal which was cut off -->
<div id="rejection-modal" class="fixed inset-0 overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true" style="z-index: 50;">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true"></div>
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        ...
                        <div class="mt-2">
                            <p class="text-sm text-gray-500" id="rejection-reason-text">
                                <!-- Reason text will be inserted here by JavaScript -->
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button type="button" id="close-rejection-modal" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                    Fermer
                </button>
            </div>
        </div>
...
<script>
    ...
    function viewDocument(documentId) {
        ...
        fetch(`/candidate/documents/${documentId}/preview`)
            .then(response => {
                if (!response.ok) {
                    throw new Error('Erreur réseau lors de la récupération du document');
                }
